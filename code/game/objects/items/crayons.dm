...
/obj/item/toy/crayon/after_attack(atom/target, mob/user, proximity_flag, click_parameters)
	...
	if(is_type_in_list(target,validSurfaces))
		...
		if(instant || do_after(user, 50 * toolspeed, target = target))
			var/obj/effect/decal/cleanable/crayon/C = new /obj/effect/decal/cleanable/crayon(target,color,drawtype,temp)
...
/obj/item/toy/crayon/mime/update_window(mob/living/user as mob)
	dat += "<center><span style='border:1px solid #161616; background-color: [color];'>&nbsp;&nbsp;&nbsp;</span><a href='byond://?src=[UID()];color=1'>Change color</a></center>"
	..()
...
/obj/item/toy/crayon/spraycan/activate_self(mob/user)
	...
	switch(choice)
		...
		if("Change Drawing")
			update_window(user)
		if("Change Color")
			color = tgui_input_color(user,"Please select a paint color.","Spray Can Color")
			if(isnull(color))
				return
			update_icon()
