	...
	if(on && user.zone_selected == "eyes")
		...
		if(M == user)	//they're using it on themselves
			...
			if(istype(H)) //robots and aliens are unaffected
				...
				else if(HAS_TRAIT(M, TRAIT_XRAY_VISION) || eyes.see_in_dark >= 8) //The mob's either got the X-RAY vision or has a tapetum lucidum (extreme nightvision, i.e. Vulp/Tajara with COLORBLIND & their monkey forms).
