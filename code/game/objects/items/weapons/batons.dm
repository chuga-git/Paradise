...
/obj/item/melee/classic_baton/proc/on_non_silicon_stun(mob/living/target, mob/living/user)
	var/armor = target.run_armor_check("chest", armor_penetration_percentage = stamina_armor_pen) // returns their chest melee armor
	var/percentage_reduction = 0
	if(ishuman(target))
		percentage_reduction = (100 - ARMOR_VALUE_TO_PERCENTAGE(armor)) / 100
	else
		percentage_reduction = (100 - armor) / 100 // converts the % into a decimal
