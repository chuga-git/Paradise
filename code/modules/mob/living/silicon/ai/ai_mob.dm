...
/proc/getHologramIcon(icon/A, safety = TRUE, color = rgb(125, 180, 225), opacity = 0.5, color_blocking = FALSE) // If safety is on, a new icon is not created.
	var/icon/flat_icon = safety ? A : new(A) // Has to be a new icon to not constantly change the same icon.
	var/icon/alpha_mask
	if(color && !color_blocking)
		flat_icon.ColorTone(color) // Let's make it bluish.
