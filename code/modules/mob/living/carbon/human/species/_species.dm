/datum/species
	...
	///Additional armor value for the species.
	var/armor = 0
...
/datum/species/proc/apply_damage(damage = 0, damagetype = BRUTE, def_zone, blocked = 0, mob/living/carbon/human/H, sharp = FALSE, obj/used_weapon, spread_damage = FALSE)
	...
	var/total_armor = blocked + armor

	switch(damagetype)
		if(BRUTE)
			var/damage_amount = ARMOR_EQUATION(damage, total_armor, brute_mod * H.physiology.brute_mod)
			if(damage_amount)
		...
		if(BURN)
			var/damage_amount = ARMOR_EQUATION(damage, total_armor, burn_mod * H.physiology.burn_mod)
			if(damage_amount)
				H.damageoverlaytemp = 20
		...
		if(TOX)
			var/damage_amount = ARMOR_EQUATION(damage, total_armor, H.physiology.tox_mod)
			H.adjustToxLoss(damage_amount)
		if(OXY)
			var/damage_amount = ARMOR_EQUATION(damage, total_armor, H.physiology.oxy_mod)
			H.adjustOxyLoss(damage_amount)
		if(CLONE)
			var/damage_amount = ARMOR_EQUATION(damage, total_armor, H.physiology.clone_mod)
			H.adjustCloneLoss(damage_amount)
		if(STAMINA)
			var/damage_amount = ARMOR_EQUATION(damage, total_armor, H.physiology.stamina_mod)
			H.adjustStaminaLoss(damage_amount)
		if(BRAIN)
			var/damage_amount = ARMOR_EQUATION(damage, total_armor, H.physiology.brain_mod)
